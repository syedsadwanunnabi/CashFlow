...
import { BANKS, CATEGORIES, type Transaction, type BankId, type CategoryId } from "@/lib/data";
    ...
    <div className="rounded-xl border border-border bg-card p-5">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3">
        <input
          ...
          className="col-span-2 rounded-lg bg-secondary border border-border px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary"
          required
        />
        <select
          value={form.type}
          onChange={e => setForm(p => ({ ...p, type: e.target.value as "sent" | "received" }))}
          className="rounded-lg bg-secondary border border-border px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary"
        >
        ...
        <select
          value={form.bank}
          onChange={e => setForm(p => ({ ...p, bank: e.target.value as BankId }))}
          className="rounded-lg bg-secondary border border-border px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary"
        >
          {(Object.keys(BANKS) as BankId[]).map(id => (
        ...
        <div className="col-span-2 flex gap-2 justify-end">
          <button type="button" onClick={() => setOpen(false)} className="rounded-lg px-4 py-2 text-sm text-muted-foreground hover:text-foreground">
            {t("cancel")}
          </button>
